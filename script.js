...
const fuzzball_x = 150,
    fuzzball_y = 590; //declare starting point for fuzzball
...
function get_random(min, max) {
    //return 'fake' random number based on inputted range
    min = Math.ceil(min);
    max = Math.floor(max);
...
function preload() {
    ...
    mysteryBlock = loadImage("assets/mysteryBlock.png");
    ...
    successSound = loadSound("assets/sounds/successSound.m4a");
}
...
function paint_assets() {
    for (let i = 0; i < crates.length; i++) {
        //loop through crates array and show each
        crates[i].show();
    }
...
function createObjects() {
    ...
    leftwall = new c_ground(0, vp_height / 2, 1, vp_height, "leftwall"); //create left wall object
